...
const FinalCustomerScreen = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    const q = query(collection(db, "leads"), where("stage", "==", "Stage 2"));
  ...
  const handleForwardToStage3 = async () => {
      if (!selectedLead) return;
      try {
          const leadRef = doc(db, 'leads', selectedLead.id);
          await updateDoc(leadRef, { stage: 'Stage 3' });
          Alert.alert("Success", "Lead has been forwarded to Stage 3.");
          closeMenu();
      } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={[styles.header, { paddingTop: insets.top }]}>
        ...
        <Text style={styles.headerTitle}>Final Customers</Text>
        <View style={{width: 36}} />
      </View>
      <View style={styles.mainContent}>
          ...
          <View style={styles.leadsListSection}>
              <View style={styles.leadsListHeader}>
                  <Text style={styles.leadsListTitle}>Final Customers ({filteredLeads.length})</Text>
                  <TouchableOpacity style={styles.createLeadButton} onPress={() => router.push('/create-final-customer')}>
                      <Feather name="plus-circle" size={16} color="#0a7ea4" />
                      <Text style={styles.createLeadButtonText}>Create Final Customer</Text>
                  </TouchableOpacity>
              </View>
                  ...
                  <FlatList
                      ...
                      ListEmptyComponent={() => (
                          <View style={styles.emptyStateContainer}>
                              <Text style={styles.emptyStateText}>No final customers found for this period.</Text>
                              <Text style={styles.emptyStateSubText}>Try selecting a different month or create a new final customer.</Text>
                          </View>
                      )}
                  />
              )}
      ...
      >
        <TouchableOpacity style={styles.modalOverlay} onPress={closeMenu} activeOpacity={1}>
            <View style={styles.menuContainer}>
                <TouchableOpacity style={styles.menuItem} onPress={handleForwardToStage3}>
...
export default FinalCustomerScreen;
