...
const EditDispatchDetailsScreen = () => {
  ...
  const [dispatch, setDispatch] = useState({ uri: null, name: null, type: null });
  const [existingDispatchURL, setExistingDispatchURL] = useState(null);
    ...
    if (id) {
      ...
      getDoc(leadRef).then(docSnap => {
        if (docSnap.exists()) {
          ...
          setStage(data.stage || '');
          setExistingDispatchURL(data.dispatchURL || null);
          if (data.assignedTo) {
  ...
  const handlePickImage = async () => {
    ...
    if (!result.canceled) {
      setDispatch({ uri: result.assets[0].uri, name: result.assets[0].uri.split('/').pop(), type: 'image' });
    }
  };
  ...
  const handlePickDocument = async () => {
    try {
        ...
        if (result.type === 'success') {
            setDispatch({ uri: result.uri, name: result.name, type: 'pdf' });
        }
    } catch (error) {
  ...
  const uploadFile = async (uri, fileName) => {
    ...
    const storageRef = ref(storage, `dispatches/${id}_${fileName}`);
  ...
  const handleUpdateDetails = async () => {
    ...
    setIsUpdating(true);
    try {
      let dispatchURL = existingDispatchURL;

      if (dispatch.uri) {
        dispatchURL = await uploadFile(dispatch.uri, dispatch.name);
      }
      ...
      Alert.alert("Success", "Dispatch has been updated successfully.");
      router.back();
    } catch (error) {
      console.error("Error updating document: ", error);
      Alert.alert("Error", "There was an error updating the dispatch. Please try again.");
    } finally {
      setIsUpdating(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={[styles.header, { paddingTop: insets.top }]}>
        ...
        <Text style={styles.headerTitle}>Edit Dispatch Details</Text>
        <View style={{width: 36}} />
      </View>
      <ScrollView contentContainerStyle={styles.mainContent}>
        ...
        <View style={styles.card}>
            ...
            {(dispatch.uri || existingDispatchURL) &&
                <View style={styles.filePreviewContainer}>
                    <Text style={styles.label}>Preview:</Text>
                    { (dispatch.type === 'image' || (!dispatch.type && existingDispatchURL && existingDispatchURL.includes('.jpg'))) ? (
                        <Image source={{ uri: dispatch.uri || existingDispatchURL }} style={styles.image} />
                    ) : (
                        <View style={styles.pdfPreview}>
                            <Feather name="file" size={40} color="#10B981" />
                            <Text style={styles.pdfName} numberOfLines={1}>{dispatch.name || existingDispatchURL?.split('F').pop().split('?')[0]}</Text>
                        </View>
                    )}
                </View>
            }
...
export default EditDispatchDetailsScreen;
