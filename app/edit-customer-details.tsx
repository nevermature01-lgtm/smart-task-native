...
const EditCustomerDetailsScreen = () => {
  ...
  const [approvalForm, setApprovalForm] = useState({ uri: null, name: null, type: null });
  const [existingApprovalFormURL, setExistingApprovalFormURL] = useState(null);
    ...
    if (id) {
      ...
      getDoc(leadRef).then(docSnap => {
        if (docSnap.exists()) {
          ...
          setStage(data.stage || '');
          setExistingApprovalFormURL(data.approvalFormURL || null);
          if (data.assignedTo) {
  ...
  const handlePickImage = async () => {
    ...
    if (!result.canceled) {
      setApprovalForm({ uri: result.assets[0].uri, name: result.assets[0].uri.split('/').pop(), type: 'image' });
    }
  };
  ...
  const handlePickDocument = async () => {
    try {
        ...
        if (result.type === 'success') {
            setApprovalForm({ uri: result.uri, name: result.name, type: 'pdf' });
        }
    } catch (error) {
  ...
  const uploadFile = async (uri, fileName) => {
    ...
    const storageRef = ref(storage, `customer_approvals/${id}_${fileName}`);
  ...
  const handleUpdateDetails = async () => {
    ...
    setIsUpdating(true);
    try {
      let approvalFormURL = existingApprovalFormURL;

      if (approvalForm.uri) {
        approvalFormURL = await uploadFile(approvalForm.uri, approvalForm.name);
      }
  ...
  if (isFetching) {
    return <ActivityIndicator style={{flex: 1, justifyContent: 'center', alignItems: 'center'}} size="large" color="#FB923C" />
  }
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={[styles.header, { paddingTop: insets.top }]}>
        ...
        <Text style={styles.headerTitle}>Edit Customer Details</Text>
        <View style={{width: 36}} />
      </View>
      <ScrollView contentContainerStyle={styles.mainContent}>
        ...
        <View style={styles.card}>
            <Text style={styles.cardTitle}>Customer Approval Form</Text>
            ...
            {(approvalForm.uri || existingApprovalFormURL) &&
                <View style={styles.filePreviewContainer}>
                    <Text style={styles.label}>Preview:</Text>
                    { (approvalForm.type === 'image' || (!approvalForm.type && existingApprovalFormURL && existingApprovalFormURL.includes('.jpg'))) ? (
                        <Image source={{ uri: approvalForm.uri || existingApprovalFormURL }} style={styles.image} />
                    ) : (
                        <View style={styles.pdfPreview}>
                            <Feather name="file" size={40} color="#FB923C" />
                            <Text style={styles.pdfName} numberOfLines={1}>{approvalForm.name || existingApprovalFormURL?.split('F').pop().split('?')[0]}</Text>
                        </View>
                    )}
                </View>
            }
                ...
                <View style={styles.modalContent}>
                    ...
                    {sources.map((item, index) => (
                        <TouchableOpacity key={index} style={styles.modalItem} onPress={() => handleSelectSource(item)}>
                            ...
                            {source === item && <Feather name="check" size={20} color="#FB923C" />} 
                        </TouchableOpacity>
                    ))}
                </View>
            </TouchableOpacity>
                    ...
                    <ScrollView>
                        {users.map((item, index) => (
                            <TouchableOpacity key={index} style={styles.modalItem} onPress={() => handleSelectUser(item)}>
                                ...
                                {selectedUsers.find(u => u.id === item.id) && <Feather name="check" size={20} color="#FB923C" />} 
                            </TouchableOpacity>
                        ))}
                    </ScrollView>
...
export default EditCustomerDetailsScreen;
