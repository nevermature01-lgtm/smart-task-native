...
const FeedbackDetailsScreen = () => {
    ...
    if (id) {
      ...
      const unsubscribe = onSnapshot(visitRef, (docSnap) => {
        if (docSnap.exists()) {
          ...
          Alert.alert("Error", "Feedback/Complaint not found.");
        }
        setIsLoading(false);
      }, error => {
  ...
  const handleCall = (phoneNumber) => {
    if (phoneNumber) {
      ...
      Alert.alert("No Contact", "No contact number is available for this feedback/complaint.");
    }
  };
  ...
  if (!visit) {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
        <Text>Feedback/Complaint not found.</Text>
      </View>
    );
  }
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={[styles.header, { paddingTop: insets.top }]}>
        ...
        <Text style={styles.headerTitle}>Feedback/Complaint Details</Text>
        <TouchableOpacity style={styles.headerButton} onPress={() => router.push(`/edit-feedback-details?id=${id}`)}>
      ...
      <ScrollView contentContainerStyle={styles.mainContent}>
        <View style={styles.detailCard}>
            ...
            {visit.feedbackURL && (
              <View style={styles.detailRow}>
                <Text style={styles.detailLabel}>Feedback/Complaint Form</Text>
                {visit.feedbackURL.includes('.pdf') ? (
                  <TouchableOpacity onPress={() => Linking.openURL(visit.feedbackURL)}>
                    <View style={styles.pdfPreview}>
                        <Feather name="file" size={40} color="#EF4444" />
                        <Text style={styles.pdfName} numberOfLines={1}>{visit.feedbackURL.split('F').pop().split('?')[0]}</Text>
                    </View>
                  </TouchableOpacity>
                ) : (
                  <Image source={{ uri: visit.feedbackURL }} style={styles.image} />
                )}
              </View>
            )}
        </View>
