...
const FeedbackScreen = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    const q = query(collection(db, "leads"), where("stage", "==", "Stage 8"));
            ...
            { 
                ...
                onPress: async () => {
                    try {
                        ...
                        Alert.alert("Success", "Feedback/Complaint has been removed.");
                        closeMenu();
                    } catch (error) {
                        Alert.alert("Error", "Failed to remove Feedback/Complaint. Please try again.");
                    }
                }
            }
        ]
  ...
  const LeadCard = ({ item }) => {
    ...
    return (
        <TouchableOpacity onPress={() => router.push(`/feedback-details?id=${item.id}`)}>
            <View style={[styles.leadCard, {backgroundColor: '#FEE2E2', borderColor: '#FCA5A5'}]}>
                <View style={styles.leadCardHeader}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                        <View style={[styles.leadAvatar, { backgroundColor: '#FECACA'}]}>
                            <Text style={[styles.leadAvatarText, { color: '#991B1B'}]}>{getInitials(item.customerName)}</Text>
                        </View>
                        <View style={{ flex: 1 }}>
                    ...
                    {item.stage && <Text style={[styles.stageBadge, {backgroundColor: '#FCA5A5', color: '#991B1B'}]}>{item.stage}</Text>}
                </View>
                <View style={[styles.leadCardBody, {borderColor: '#FCA5A5'}]}>
                ...
                <View style={styles.leadCardFooter}>
                    ...
                    <View style={styles.leadActions}>
                        <TouchableOpacity style={[styles.leadActionButton, { backgroundColor: 'rgba(239, 68, 68, 0.1)' }]} onPress={() => handleCall(item.contactNumber)}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={[styles.header, { paddingTop: insets.top }]}>
        ...
        <Text style={styles.headerTitle}>Feedback/Complaint</Text>
        <View style={{width: 36}} />
      </View>
      <View style={styles.mainContent}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.quickFilters}>
              <TouchableOpacity style={[styles.filterChip, styles.activeFilterChip, {backgroundColor: '#EF4444', borderColor: '#EF4444'}]}>
          ...
          <View style={styles.leadsListSection}>
              <View style={styles.leadsListHeader}>
                  <Text style={styles.leadsListTitle}>Feedbacks/Complaints ({filteredLeads.length})</Text>
              </View>
                  ...
                  <FlatList
                      ...
                      ListEmptyComponent={() => (
                          <View style={styles.emptyStateContainer}>
                              <Text style={styles.emptyStateText}>No feedbacks or complaints found for this period.</Text>
      ...
      >
        <TouchableOpacity style={styles.modalOverlay} onPress={closeMenu} activeOpacity={1}>
            <View style={styles.menuContainer}>
                <TouchableOpacity style={styles.menuItem} onPress={() => { router.push(`/edit-feedback-details?id=${selectedLead.id}`); closeMenu(); }}>
                ...
                <TouchableOpacity style={[styles.menuItem, styles.destructiveMenuItem]} onPress={handleRemoveLead}>
                    ...
                    <Text style={[styles.menuItemText, styles.destructiveMenuItemText]}>Remove Feedback/Complaint</Text>
                </TouchableOpacity>
            </View>
        </TouchableOpacity>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  leadCard: { backgroundColor: '#FEE2E2', borderRadius: 16, padding: 16, marginBottom: 16, borderWidth: 1, borderColor: '#FCA5A5' },
  ...
  stageBadge: { backgroundColor: '#FCA5A5', color: '#991B1B', fontSize: 10, fontWeight: 'bold', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 999, overflow: 'hidden' },
  leadCardBody: { flexDirection: 'row', gap: 16, paddingVertical: 12, borderTopWidth: 1, borderBottomWidth: 1, borderColor: '#FCA5A5' },
  leadStat: { flex: 1 },
