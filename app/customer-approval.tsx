...
const CustomerApprovalScreen = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    const q = query(collection(db, "leads"), where("stage", "==", "Stage 4"));
            ...
            { 
                ...
                onPress: async () => {
                    try {
                        ...
                        Alert.alert("Success", "Customer approval has been removed.");
                        closeMenu();
                    } catch (error) {
                        Alert.alert("Error", "Failed to remove customer approval. Please try again.");
                    }
                }
            }
        ]
  ...
  const handleForwardToStage5 = async () => {
      if (!selectedLead) return;
      try {
          const leadRef = doc(db, 'leads', selectedLead.id);
          await updateDoc(leadRef, { stage: 'Stage 5' });
          Alert.alert("Success", "Lead has been forwarded to Stage 5.");
          closeMenu();
      } catch (error) {
  ...
  const LeadCard = ({ item }) => {
    ...
    return (
        <TouchableOpacity onPress={() => router.push(`/customer-approval-details?id=${item.id}`)}>
            <View style={styles.leadCard}>
                <View style={styles.leadCardHeader}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                        <View style={[styles.leadAvatar, { backgroundColor: '#FFF7ED'}]}>
                            <Text style={[styles.leadAvatarText, { color: '#FB923C'}]}>{getInitials(item.customerName)}</Text>
                        </View>
                        <View style={{ flex: 1 }}>
                ...
                <View style={styles.leadCardFooter}>
                    ...
                    <View style={styles.leadActions}>
                        <TouchableOpacity style={[styles.leadActionButton, { backgroundColor: 'rgba(251, 146, 60, 0.1)' }]} onPress={() => handleCall(item.contactNumber)}>
                            <Feather name="phone" size={18} color="#FB923C" />
                        </TouchableOpacity>
      ...
      <View style={styles.mainContent}>
          ...
          <View style={styles.leadsListSection}>
              <View style={styles.leadsListHeader}>
                  <Text style={styles.leadsListTitle}>Customer Approvals ({filteredLeads.length})</Text>
              </View>

              {isLoading ? (
                  <ActivityIndicator size="large" color="#FB923C" style={{marginTop: 50}} />
              ) : (
                  <FlatList
                      data={filteredLeads}
                      renderItem={LeadCard}
                      ...
                      ListEmptyComponent={() => (
                          <View style={styles.emptyStateContainer}>
                              <Text style={styles.emptyStateText}>No customer approvals found for this period.</Text>
      ...
      >
        <TouchableOpacity style={styles.modalOverlay} onPress={closeMenu} activeOpacity={1}>
            <View style={styles.menuContainer}>
                <TouchableOpacity style={styles.menuItem} onPress={handleForwardToStage5}>
                    <Feather name="send" size={20} color="#4B5563" />
                    <Text style={styles.menuItemText}>Forward to Stage 5</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.menuItem} onPress={() => { router.push(`/edit-customer-details?id=${selectedLead.id}`); closeMenu(); }}>
                ...
                <TouchableOpacity style={[styles.menuItem, styles.destructiveMenuItem]} onPress={handleRemoveLead}>
                    ...
                    <Text style={[styles.menuItemText, styles.destructiveMenuItemText]}>Remove Customer Approval</Text>
                </TouchableOpacity>
            </View>
        </TouchableOpacity>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  activeFilterChip: { backgroundColor: '#FB923C', borderColor: '#FB923C' },
  ...
  leadCard: { backgroundColor: '#FFF7ED', borderRadius: 16, padding: 16, marginBottom: 16, borderWidth: 1, borderColor: '#FDBA74' },
  ...
  stageBadge: { backgroundColor: '#FED7AA', color: '#C2410C', fontSize: 10, fontWeight: 'bold', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 999, overflow: 'hidden' },
  leadCardBody: { flexDirection: 'row', gap: 16, paddingVertical: 12, borderTopWidth: 1, borderBottomWidth: 1, borderColor: '#FDBA74' },
  leadStat: { flex: 1 },
