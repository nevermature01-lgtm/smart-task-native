...
const EditWorkCompleteDetailsScreen = () => {
  ...
  const [workComplete, setWorkComplete] = useState({ uri: null, name: null, type: null });
  const [existingWorkCompleteURL, setExistingWorkCompleteURL] = useState(null);
    ...
    if (id) {
      ...
      getDoc(leadRef).then(docSnap => {
        if (docSnap.exists()) {
          ...
          setStage(data.stage || '');
          setExistingWorkCompleteURL(data.workCompleteURL || null);
          if (data.assignedTo) {
  ...
  const handlePickImage = async () => {
    ...
    if (!result.canceled) {
      setWorkComplete({ uri: result.assets[0].uri, name: result.assets[0].uri.split('/').pop(), type: 'image' });
    }
  };
  ...
  const handlePickDocument = async () => {
    try {
        ...
        if (result.type === 'success') {
            setWorkComplete({ uri: result.uri, name: result.name, type: 'pdf' });
        }
    } catch (error) {
  ...
  const uploadFile = async (uri, fileName) => {
    ...
    const storageRef = ref(storage, `work-completes/${id}_${fileName}`);
  ...
  const handleUpdateDetails = async () => {
    ...
    setIsUpdating(true);
    try {
      let workCompleteURL = existingWorkCompleteURL;

      if (workComplete.uri) {
        workCompleteURL = await uploadFile(workComplete.uri, workComplete.name);
      }
      ...
      Alert.alert("Success", "Work Complete has been updated successfully.");
      router.back();
    } catch (error) {
      console.error("Error updating document: ", error);
      Alert.alert("Error", "There was an error updating the work complete. Please try again.");
    } finally {
      setIsUpdating(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={[styles.header, { paddingTop: insets.top }]}>
        ...
        <Text style={styles.headerTitle}>Edit Work Complete Details</Text>
        <View style={{width: 36}} />
      </View>
      <ScrollView contentContainerStyle={styles.mainContent}>
        ...
        <View style={styles.card}>
            <Text style={styles.cardTitle}>Work Complete</Text>
            ...
            {(workComplete.uri || existingWorkCompleteURL) &&
                <View style={styles.filePreviewContainer}>
                    <Text style={styles.label}>Preview:</Text>
                    { (workComplete.type === 'image' || (!workComplete.type && existingWorkCompleteURL && existingWorkCompleteURL.includes('.jpg'))) ? (
                        <Image source={{ uri: workComplete.uri || existingWorkCompleteURL }} style={styles.image} />
                    ) : (
                        <View style={styles.pdfPreview}>
                            <Feather name="file" size={40} color="#10B981" />
                            <Text style={styles.pdfName} numberOfLines={1}>{workComplete.name || existingWorkCompleteURL?.split('F').pop().split('?')[0]}</Text>
                        </View>
                    )}
                </View>
            }
...
export default EditWorkCompleteDetailsScreen;
