...
const ProjectDetailFoamScreen = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    const q = query(collection(db, "leads"), where("stage", "==", "Stage 5"));
            ...
            { 
                ...
                onPress: async () => {
                    try {
                        ...
                        Alert.alert("Success", "Project detail foam has been removed.");
                        closeMenu();
                    } catch (error) {
                        Alert.alert("Error", "Failed to remove project detail foam. Please try again.");
                    }
                }
            }
        ]
  ...
  const handleForwardToStage6 = async () => {
      if (!selectedLead) return;
      try {
          const leadRef = doc(db, 'leads', selectedLead.id);
          await updateDoc(leadRef, { stage: 'Stage 6' });
          Alert.alert("Success", "Lead has been forwarded to Stage 6.");
          closeMenu();
      } catch (error) {
  ...
  const LeadCard = ({ item }) => {
    ...
    return (
        <TouchableOpacity onPress={() => router.push(`/project-foam-details?id=${item.id}`)}>
      ...
      <View style={styles.mainContent}>
          ...
          <View style={styles.leadsListSection}>
              <View style={styles.leadsListHeader}>
                  <Text style={styles.leadsListTitle}>Project Detail Foams ({filteredLeads.length})</Text>
              </View>
                  ...
                  <FlatList
                      ...
                      ListEmptyComponent={() => (
                          <View style={styles.emptyStateContainer}>
                              <Text style={styles.emptyStateText}>No project detail foams found for this period.</Text>
      ...
      >
        <TouchableOpacity style={styles.modalOverlay} onPress={closeMenu} activeOpacity={1}>
            <View style={styles.menuContainer}>
                <TouchableOpacity style={styles.menuItem} onPress={handleForwardToStage6}>
                    <Feather name="send" size={20} color="#4B5563" />
                    <Text style={styles.menuItemText}>Forward to Stage 6</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.menuItem} onPress={() => { router.push(`/edit-project-foam-details?id=${selectedLead.id}`); closeMenu(); }}>
                ...
                <TouchableOpacity style={[styles.menuItem, styles.destructiveMenuItem]} onPress={handleRemoveLead}>
                    ...
                    <Text style={[styles.menuItemText, styles.destructiveMenuItemText]}>Remove Project Detail Foam</Text>
                </TouchableOpacity>
            </View>
        </TouchableOpacity>
      </Modal>
