...
const WorkCompleteScreen = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    const q = query(collection(db, "leads"), where("stage", "==", "Stage 7"));
            ...
            { 
                ...
                onPress: async () => {
                    try {
                        ...
                        Alert.alert("Success", "Work Complete has been removed.");
                        closeMenu();
                    } catch (error) {
                        Alert.alert("Error", "Failed to remove Work Complete. Please try again.");
                    }
                }
            }
        ]
  ...
  const handleForwardToStage8 = async () => {
      if (!selectedLead) return;
      try {
          const leadRef = doc(db, 'leads', selectedLead.id);
          await updateDoc(leadRef, { stage: 'Stage 8' });
          Alert.alert("Success", "Lead has been forwarded to Stage 8.");
          closeMenu();
      } catch (error) {
  ...
  const LeadCard = ({ item }) => {
    ...
    return (
        <TouchableOpacity onPress={() => router.push(`/work-complete-details?id=${item.id}`)}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={[styles.header, { paddingTop: insets.top }]}>
        ...
        <Text style={styles.headerTitle}>Work Complete</Text>
        <View style={{width: 36}} />
      </View>
      <View style={styles.mainContent}>
          ...
          <View style={styles.leadsListSection}>
              <View style={styles.leadsListHeader}>
                  <Text style={styles.leadsListTitle}>Work Completes ({filteredLeads.length})</Text>
              </View>
                  ...
                  <FlatList
                      ...
                      ListEmptyComponent={() => (
                          <View style={styles.emptyStateContainer}>
                              <Text style={styles.emptyStateText}>No work completes found for this period.</Text>
      ...
      >
        <TouchableOpacity style={styles.modalOverlay} onPress={closeMenu} activeOpacity={1}>
            <View style={styles.menuContainer}>
                <TouchableOpacity style={styles.menuItem} onPress={handleForwardToStage8}>
                    <Feather name="send" size={20} color="#4B5563" />
                    <Text style={styles.menuItemText}>Forward to Stage 8</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.menuItem} onPress={() => { router.push(`/edit-work-complete-details?id=${selectedLead.id}`); closeMenu(); }}>
                ...
                <TouchableOpacity style={[styles.menuItem, styles.destructiveMenuItem]} onPress={handleRemoveLead}>
                    ...
                    <Text style={[styles.menuItemText, styles.destructiveMenuItemText]}>Remove Work Complete</Text>
                </TouchableOpacity>
            </View>
        </TouchableOpacity>
      </Modal>
