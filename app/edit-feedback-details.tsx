...
const EditFeedbackDetailsScreen = () => {
  ...
  const [feedback, setFeedback] = useState({ uri: null, name: null, type: null });
  const [existingFeedbackURL, setExistingFeedbackURL] = useState(null);
    ...
    if (id) {
      ...
      getDoc(leadRef).then(docSnap => {
        if (docSnap.exists()) {
          ...
          setStage(data.stage || '');
          setExistingFeedbackURL(data.feedbackURL || null);
          if (data.assignedTo) {
  ...
  const handlePickImage = async () => {
    ...
    if (!result.canceled) {
      setFeedback({ uri: result.assets[0].uri, name: result.assets[0].uri.split('/').pop(), type: 'image' });
    }
  };
  ...
  const handlePickDocument = async () => {
    try {
        ...
        if (result.type === 'success') {
            setFeedback({ uri: result.uri, name: result.name, type: 'pdf' });
        }
    } catch (error) {
  ...
  const uploadFile = async (uri, fileName) => {
    ...
    const storageRef = ref(storage, `feedbacks/${id}_${fileName}`);
  ...
  const handleUpdateDetails = async () => {
    ...
    setIsUpdating(true);
    try {
      let feedbackURL = existingFeedbackURL;

      if (feedback.uri) {
        feedbackURL = await uploadFile(feedback.uri, feedback.name);
      }
      ...
      Alert.alert("Success", "Feedback/Complaint has been updated successfully.");
      router.back();
    } catch (error) {
      console.error("Error updating document: ", error);
      Alert.alert("Error", "There was an error updating the feedback/complaint. Please try again.");
    } finally {
      setIsUpdating(false);
    }
  };
  ...
  if (isFetching) {
    return <ActivityIndicator style={{flex: 1, justifyContent: 'center', alignItems: 'center'}} size="large" color="#EF4444" />
  }
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={[styles.header, { paddingTop: insets.top }]}>
        ...
        <Text style={styles.headerTitle}>Edit Feedback/Complaint Details</Text>
        <View style={{width: 36}} />
      </View>
      <ScrollView contentContainerStyle={styles.mainContent}>
        ...
        <View style={styles.card}>
            <Text style={styles.cardTitle}>Feedback/Complaint</Text>
            ...
            {(feedback.uri || existingFeedbackURL) &&
                <View style={styles.filePreviewContainer}>
                    <Text style={styles.label}>Preview:</Text>
                    { (feedback.type === 'image' || (!feedback.type && existingFeedbackURL && existingFeedbackURL.includes('.jpg'))) ? (
                        <Image source={{ uri: feedback.uri || existingFeedbackURL }} style={styles.image} />
                    ) : (
                        <View style={styles.pdfPreview}>
                            <Feather name="file" size={40} color="#EF4444" />
                            <Text style={styles.pdfName} numberOfLines={1}>{feedback.name || existingFeedbackURL?.split('F').pop().split('?')[0]}</Text>
                        </View>
                    )}
                </View>
            }
                ...
                <View style={styles.modalContent}>
                    ...
                    {sources.map((item, index) => (
                        <TouchableOpacity key={index} style={styles.modalItem} onPress={() => handleSelectSource(item)}>
                            ...
                            {source === item && <Feather name="check" size={20} color="#EF4444" />} 
                        </TouchableOpacity>
                    ))}
                </View>
            </TouchableOpacity>
                    ...
                    <ScrollView>
                        {users.map((item, index) => (
                            <TouchableOpacity key={index} style={styles.modalItem} onPress={() => handleSelectUser(item)}>
                                ...
                                {selectedUsers.find(u => u.id === item.id) && <Feather name="check" size={20} color="#EF4444" />} 
                            </TouchableOpacity>
                        ))}
                    </ScrollView>
                     <TouchableOpacity style={[styles.doneButton, {backgroundColor: '#EF4444'}]} onPress={() => setUserModalVisible(false)}>
...
export default EditFeedbackDetailsScreen;
